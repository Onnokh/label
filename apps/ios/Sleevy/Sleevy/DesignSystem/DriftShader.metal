...
fragment float4 drift_fragment(
    ...
    nearTint = mix(nearTint, driftPink, smoothstep(0.55, 0.90, pinkPass) * 0.35);
    ...
    if (lightMode > 0.5) {
        ...
        float m0 = 0.14;
        premul = driftPeriwinkle * m0 + premul * (1.0 - m0);
        alpha = m0 + alpha * (1.0 - m0);

        float m1 = far * 0.10;
        premul = driftBlue * m1 + premul * (1.0 - m1);
    ...
    light += driftBlue * 0.06;
    light += driftPeriwinkle * (far * 0.18);
    light += nearTint * (near * 0.30);
